...
$likeCount = 0;
if(isset($_POST['ajaxId']) && is_numeric($_POST['ajaxId']) && isset($_POST['count']) && is_numeric($_POST['count'])){
    $id = $_POST['ajaxId'];
    $newCount = $_POST['count']+1;
    $insert = mysql_query("update plans set votes=".$newCount." where id=".$id);
}
    
$sql = mysql_query("select * from plans where id = ".$id);
if($sql){
    $row = mysql_fetch_row($sql);
    $result = array();
    foreach ($row as $key => $val){
        $result[mysql_field_name($sql,$key)] = $val;
    }
    
    if(isset($result) && !empty($result) && is_array($result)){
        $likeCount = $result['votes'];
    }
		...
		<script type="text/javascript" src="js/jquery.min.js"></script>
                <script type="text/javascript">
                        $(document).ready(function(){
                                $("#rating_comment-like").click(function(){
                                    var id = $(this).attr("value");
                                    var count = $("#rating-like").html();
                                    var dataSend = 'ajaxId='+ id +'&count='+count;	
                                    $.ajax({
                                           type : "POST",
                                           data : dataSend,
                                           success : function(html){
                                                   $("#rating-like").text(html);
                                           }
                                    });
                                });
                        });
					...
					<div class="row">
						<div class="12u">
							<div class="rating_comment">
                                                            <a href="#" id="rating_comment-like" style="rating_comment-like" onclick="return false;" value="<?php echo isset($result["id"]) ? $result["id"] : 0; ?>">
									<img src="images/rating_comment-like.png">
								</a>
								<span id="rating-like" class="rating-like"><?php echo $likeCount; ?></span>												
